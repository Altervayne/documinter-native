...
export function mkBlock(type: BlockType): Block {
   ...
   switch (type) {
      ...
      case 'list':      return { id, type, items: [{ id: crypto.randomUUID(), text: 'First item', children: [] }, { id: crypto.randomUUID(), text: 'Second item', children: [] }] }
...
export function updateListItemText(items: ListItem[], itemId: string, text: string): ListItem[] {
   return mutateListItem(items, itemId, item => ({ ...item, text }))
}
