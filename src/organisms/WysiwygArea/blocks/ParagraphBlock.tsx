...
export function ParagraphBlock({ block, patch }: ParagraphBlockProps) {
   const { t } = useLang()
   const tag = block.type as 'p' | 'h3' | 'h4'
   const placeholder = tag === 'h3' ? t.blockH3 : tag === 'h4' ? t.blockH4 : t.clickToEdit
   return (
      <ContentEditable tag={tag} content={block.text ?? ''} onBlur={value => patch({ text: value })} rich placeholder={placeholder} />
   )
}
